...
# The only characters an answer is allowed to contain.
ANSWER_CHARACTERS = "0123456789-/."
...
# Return the reason a typed answer cannot be marked, or "" if it can be.
def check_answer(typed):
    answer = typed.strip()

    if answer == "":
        return "You have not typed an answer. Type your answer into the box and then press Submit."

    for letter in answer:
        if letter not in ANSWER_CHARACTERS:
            return "Your answer contains the character " + letter + ", which is not allowed. Answers can only use digits, a minus sign, a slash for fractions, or a decimal point."
...
class Question:
    ...
    # Compare a typed answer against the right one, ignoring stray spaces.
