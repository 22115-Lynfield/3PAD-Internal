...
# Pictures already loaded, so the same file is never opened and resized twice.
loaded_images = {}
...
driving_help = "Left and right arrows to pick a lane, then Space or Enter to drive through it. Click the pen for a working out pad."
...
def wrong_answers(answer):
    ...
    if "/" in answer:
        ...
        top = int(top)
        bottom = int(bottom)
        guesses = [f"{top + 1}/{bottom}" , f"{top}/{bottom + 2}" , f"{bottom}/{top}" , f"{top + 2}/{bottom}"]
...
def make_addition():
    ...
    return build_question(f"What is {first} + {second}?" , str(first + second) , "Addition")
...
def make_subtraction():
    ...
    return build_question(f"What is {first} - {second}?" , str(first - second) , "Subtraction")

# Makes a times table question.
def make_multiplication():
    first = random.randint(2 , 12)
    second = random.randint(2 , 12)
    return build_question(f"What is {first} x {second}?" , str(first * second) , "Multiplication")

# Makes a division question from the answer, which keeps it exact.
def make_division():
    divisor = random.randint(2 , 12)
    answer = random.randint(2 , 12)
    return build_question(f"What is {divisor * answer} / {divisor}?" , str(answer) , "Division")
...
def make_exponents():
    ...
    return build_question(f"What is {base} to the power of {power}?" , str(base ** power) , "Exponents")

# Makes a square root question by squaring the answer first.
def make_square_root():
    answer = random.randint(2 , 30)
    return build_question(f"What is the square root of {answer * answer}?" , str(answer) , "Square Roots")
...
def make_fraction():
    ...
    question = f"What is {first}/{bottom} + {second}/{bottom}? Give your answer as a fraction in its simplest form."
...
def make_percentage():
    ...
    return build_question(f"What is {percent}% of {amount}?" , str(percent * amount // 100) , "Percentages")
...
def make_prime():
    ...
    return build_question(f"What is the next prime number after {start}?" , str(answer) , "Prime Numbers")
...
def make_factor():
    ...
    return build_question(f"What is the highest common factor of {first} and {second}?" , str(highest_common_factor(first , second)) , "Highest Common Factor")
...
def make_multiple():
    ...
    return build_question(f"What is the lowest common multiple of {first} and {second}?" , str(answer) , "Lowest Common Multiple")
...
def make_algebra():
    ...
    question = f"Solve for x:   {times}x + {plus} = {times * answer + plus}"
...
def make_area():
    ...
    question = f"A rectangle is {width} cm wide and {height} cm tall. What is its area in square cm?"
...
def make_perimeter():
    width = random.randint(3 , 15)
    height = random.randint(3 , 15)
    question = f"A rectangle is {width} cm wide and {height} cm tall. What is its perimeter in cm?"
...
def make_volume():
    ...
    question = f"A box is {length} cm long, {width} cm wide and {height} cm tall. What is its volume in cubic cm?"
...
def make_mean():
    ...
    question = f"What is the mean of {listed}?"
...
def make_median():
    ...
    listed = ", ".join(str(number) for number in numbers)
    question = f"What is the median of {listed}?"
...
def make_mode():
    ...
    listed = ", ".join(str(number) for number in numbers)
    question = f"What is the mode of {listed}?"
...
def make_differentiation():
    ...
    question = f"y = {coefficient}x^{power}.   What is dy/dx when x = {at}?"
...
def make_integration():
    ...
    if power == 1:
        equation = f"{coefficient}x"
    else:
        equation = f"{coefficient}x^{power}"

    question = f"What is the area under y = {equation} between x = 0 and x = {upper}?"
...
# Shows an error message box saying what went wrong and how to fix it.
...
def apply_theme():
    ...
    global pad_color , pad_ink , zone_colors
...
# Returns a picture at the size asked for, loading it only the first time.
def load_image(name , size):
    key = f"{name}{size}"

    if key not in loaded_images:
        # A missing picture file leaves an empty space instead of stopping the game.
        try:
            loaded_images[key] = ImageTk.PhotoImage(Image.open(file_path(name)).resize(size))
...
def load_back_image():
    ...
    # One arrow is kept for each theme, because it is repainted to match.
    key = f"Back{current_theme}"

    if key not in loaded_images:
        # A missing arrow file leaves the button reading as words on their own.
        try:
            ...
            loaded_images[key] = ImageTk.PhotoImage(picture)
        except FileNotFoundError:
            loaded_images[key] = ""

    back_img = loaded_images[key]
    return back_img

# Creating the sign in screen, which doubles as the create account screen.
# The same three boxes serve both jobs, so there is only one form to keep working.
def main():
    clear_screen()

    # The form always opens ready to sign in, even after signing out of an account.
    global login_mode
    login_mode = "Sign In"
    ...
    rules = canvas.create_text(325 , rules_row , text = f"Your password must be at least {password_min} characters long, and include at least 1 number and 1 capital letter." , font = (font , 9) , fill = dim_text , width = 420)
    ...
    def create_account():
        ...
        if len(username) < username_min:
            show_error("Cannot Create Account" , f"Your username is only {len(username)} characters long. Please make it at least {username_min} characters.")
            return

        if len(username) > username_max:
            show_error("Cannot Create Account" , f"Your username is {len(username)} characters long, which is too long. Please shorten it to {username_max} characters or fewer.")
            return
        ...
        if username in load_accounts():
            show_error("Cannot Create Account" , f"The username {username} is already taken. Please choose a different username.")
            return
        ...
        if len(password) < password_min:
            ...
            show_error("Cannot Create Account" , f"Your password is only {len(password)} characters long. Please add {short_by} more so it is at least {password_min} characters.")
            return
        ...
        show_info("Account Created" , f"Welcome {username}. Your account has been saved, so next time you can just sign in.")
        open_menu(username)
    ...
    def sign_in():
        ...
        if username not in accounts:
            show_error("Cannot Sign In" , f"There is no account saved with the username {username}. Please check your spelling, or use Create an account instead to make a new one.")
            return

        salt , scrambled = accounts[username]

        if hash_password(password , salt) != scrambled:
            show_error("Cannot Sign In" , f"That password does not match the one saved for {username}. Please try typing it again.")
            return

        show_info("Signed In" , f"Welcome back {username}.")
        open_menu(username)
...
def menu_screen():
    ...
    canvas.create_text(325 , 620 , text = f"Signed in as {current_user}" , font = (font , 9) , fill = dim_text)
...
# Difficulty screen function shows a speedometer whose needle sweeps to the chosen level.
def difficulty_screen():
    ...
    canvas.create_text(132 , 306 , text = f"{points['Easy']} points an answer" , font = (font , 9) , fill = dim_text , anchor = E)
    canvas.create_text(325 , 166 , text = "MEDIUM" , font = (title_font , 24) , fill = zone_colors["Medium"] , anchor = S)
    canvas.create_text(325 , 190 , text = f"{points['Medium']} points an answer" , font = (font , 9) , fill = dim_text , anchor = S)
    canvas.create_text(518 , 282 , text = "HARD" , font = (title_font , 24) , fill = zone_colors["Hard"] , anchor = W)
    canvas.create_text(518 , 306 , text = f"{points['Hard']} points an answer" , font = (font , 9) , fill = dim_text , anchor = W)
    ...
    def show_needle():
        canvas.itemconfig(chosen , text = f"{current_difficulty.upper()}  -  {points[current_difficulty]} POINTS AN ANSWER")
...
def settings_screen():
    ...
    canvas.create_text(325 , 272 , text = f"Light mode is easier to read in a bright room. Every race is {questions_per_race} questions." , font = (font , 9) , fill = dim_text)
    ...
    canvas.create_window(450 , 420 , anchor = CENTER , window = save_button)

    back_button = Button(main_screen , text = " Back" , font = (font , 11 , "bold") , image = load_back_image() , compound = LEFT , command = menu_screen , fg = button_text , bg = button_color , activebackground = button_active , relief = FLAT , padx = 8 , pady = 6)
    canvas.create_window(150 , 420 , anchor = CENTER , window = back_button)
...
def leaderboard_screen():
    ...
    for level in topics:
        ...
        for driver , score , seconds in rows:
            ...
            canvas.create_text(left + name_offset , y , text = f"{place + 1}  {driver}" , font = (font , 9) , fill = row_color , anchor = W)
...
class Race:
    ...
    def show_question(self):
        ...
        self.bike_img = load_image("Bike.png" , (100 , 100))
    ...
    # Builds the working out pad, its three tools and the pen that opens it.
    def build_pad(self):
        # A strip behind the tools so they do not float over the gates.
        self.pad_bar = self.canvas.create_rectangle(pad_left , pad_tools_y - 18 , pad_left + pad_width , pad_top , fill = panel_color , outline = accent , width = 2)

        self.pad = Canvas(main_screen , width = pad_width , height = pad_height , bg = pad_color , highlightthickness = 2 , highlightbackground = accent)
        self.pad_window = self.canvas.create_window(pad_left , pad_top , anchor = NW , window = self.pad)

        # Following the tkinter whiteboard pattern, a press records where the stroke starts.
        self.pad.bind("<Button-1>" , self.pad_press)
        self.pad.bind("<B1-Motion>" , self.pad_drag)

        self.last_x = 0
        self.last_y = 0

        self.pen_tool = Button(main_screen , text = "Pen" , font = (font , 10 , "bold") , width = 8 , command = self.use_pen , fg = button_text , bg = button_color , activebackground = button_active , relief = FLAT)
        self.pen_tool_window = self.canvas.create_window(170 , pad_tools_y , anchor = CENTER , window = self.pen_tool)

        self.eraser_tool = Button(main_screen , text = "Eraser" , font = (font , 10 , "bold") , width = 8 , command = self.use_eraser , fg = button_text , bg = button_color , activebackground = button_active , relief = FLAT)
        self.eraser_tool_window = self.canvas.create_window(325 , pad_tools_y , anchor = CENTER , window = self.eraser_tool)

        clear_tool = Button(main_screen , text = "Clear" , font = (font , 10 , "bold") , width = 8 , command = self.clear_pad , fg = button_text , bg = button_color , activebackground = button_active , relief = FLAT)
        self.clear_tool_window = self.canvas.create_window(480 , pad_tools_y , anchor = CENTER , window = clear_tool)

        # The pen button stays on the road whether the pad is open or shut.
        pad_toggle = Button(main_screen , text = "✏️" , font = (emoji_font , 13) , width = 3 , command = self.toggle_pad , fg = button_text , bg = button_color , activebackground = button_active , relief = FLAT)
        self.canvas.create_window(pen_button_x , bottom_row_y , anchor = CENTER , window = pad_toggle)
    ...
    def toggle_pad(self):
        self.pad_open = not self.pad_open
        self.show_pad()

    # Shows or hides the pad and the three tools that go with it.
    def show_pad(self):
        if self.pad_open:
            showing = NORMAL
        else:
        ...
        for item in [self.pad_bar , self.pad_window , self.pen_tool_window , self.eraser_tool_window , self.clear_tool_window]:
            self.canvas.itemconfig(item , state = showing)

    # Lights up whichever of the two tools is currently in use.
    def pick_tool(self):
        if self.pad_tool == "Pen":
    ...
    def pad_press(self , event):
        self.last_x = event.x
        self.last_y = event.y

    # Joins the last point to the new one, or rubs out a square with the eraser.
    def pad_drag(self , event):
        if self.pad_tool == "Pen":
            self.pad.create_line(self.last_x , self.last_y , event.x , event.y , fill = pad_ink , width = pad_line_width , capstyle = ROUND)
        else:
            self.pad.create_rectangle(event.x - pad_eraser_size , event.y - pad_eraser_size , event.x + pad_eraser_size , event.y + pad_eraser_size , fill = pad_color , outline = pad_color)

        self.last_x = event.x
        self.last_y = event.y

    # Returns how long the race has taken so far, or the final time once it is over.
    def seconds(self):
    ...
    # Tick function refreshes the clock once a second for as long as the race is being played.
    def tick(self):
    ...
    def show_scoreboard(self):
        ...
        self.canvas.itemconfig(self.scoreboard , text = f"{self.difficulty.upper()}    QUESTION {showing} OF {self.total}    SCORE {self.score}    {accuracy}% CORRECT    TIME {format_time(self.seconds())}")
    ...
    def arrive(self):
        ...
        # Stopping the clock before the score line is refreshed shows the real time.
        if self.question_no >= self.total:
    ...
    def end(self):
        ...
        stats = [["DIFFICULTY" , self.difficulty] , ["CORRECT" , f"{self.correct} of {self.total}"] , ["ACCURACY" , f"{accuracy}%"] , ["TIME" , format_time(self.taken)]]
        column = 0
...
def load_accounts():
    ...
    try:
        with open(file_path("accounts.txt") , "r" , encoding = "utf-8") as file:
            for line in file:
...
def save_account(username , password):
    ...
    with open(file_path("accounts.txt") , "a" , encoding = "utf-8") as file:
        file.write(f"{username},{salt},{hash_password(password , salt)}\n")

# Saves one finished race onto the end of the records file.
def save_record(difficulty , username , score , seconds):
    with open(file_path("records.txt") , "a" , encoding = "utf-8") as file:
...
def best_records(difficulty , how_many):
    ...
    try:
        with open(file_path("records.txt") , "r" , encoding = "utf-8") as file:
            for line in file:
                parts = line.strip().split(",")
...
def load_settings():
    ...
    try:
        with open(file_path("settings.txt") , "r" , encoding = "utf-8") as file:
            for line in file:
                # Every line is stored as name=value, for example theme=Dark.
                parts = line.strip().split("=")
...
# Writes the settings back out, replacing whatever was there before.
def save_settings():
    with open(file_path("settings.txt") , "w" , encoding = "utf-8") as file:
