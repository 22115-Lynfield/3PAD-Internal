...
# Every finished race is written here so the leaderboard survives between runs.
...
# What to say at the end, listed worst last so the first match wins. Each
# pair is the lowest accuracy that earns that wording.
RESULT_MESSAGES = [(90, "Superb driving. Almost everything right."),
...
# Write a number of seconds as minutes and seconds, for example 2:05.
...
def save_record(difficulty, username, score, seconds):
    with open(RECORDS_FILE, "a") as records_file:
        records_file.write(difficulty + "," + username + "," + str(score) + "," + str(seconds) + "\n")

# Read every saved race out of the records file, skipping damaged rows.
def load_records():
    records = []

    try:
        with open(RECORDS_FILE) as records_file:
            for line in records_file:

                # Each line is difficulty,username,score,seconds.
                parts = line.strip().split(",")

                if len(parts) != 4:
                    continue

                # A row is only trusted if the level is real and both numbers
                # are actually numbers, so one bad line cannot break the board.
...
# Sort key that puts the highest score first, and among equal scores puts the
# quickest time first. The score is negated because sorting counts upwards.
...
def best_records(difficulty, how_many):
    ...
    for record in load_records():
        ...
        name = record[1]
        if name not in best or record_rank(record) < record_rank(best[name]):
            best[name] = record

    ranked = list(best.values())
    ranked.sort(key=record_rank)
    return ranked[:how_many]

# Pick the closing wording that matches how accurate the race was.
...
class Race:
    ...
    def __init__(self, difficulty, total):
        ...
        # The clock starts as soon as the race is built, and taken holds the
        # final time once the last question has been answered.
    ...
    # How long the race took, or how long it has been running so far.
    def seconds(self):
        if self.is_finished():
            return self.taken
...
# Function for the results window, shown once a race has been completed.
def results(finished):

    results_win = tk.Toplevel(root)
    results_win.title("Race Results")
    results_win.geometry("700x700")

    # Set up the canvas widget for the results window.
    results_canvas = Canvas(results_win, width=650, height=650, bg=BACKGROUND, highlightthickness=0)
    results_canvas.pack(pady=5)
    heading_shadow = results_canvas.create_text(328, 74, text="RACE COMPLETE", font=HEADING_FONT, fill=SHADOW)
    heading_txt = results_canvas.create_text(325, 70, text="RACE COMPLETE", font=HEADING_FONT, fill=ACCENT)

    # A card holding everything the driver earned.
    results_panel = results_canvas.create_rectangle(70, 140, 580, 470, fill=PANEL, outline=PANEL_EDGE, width=2)

    # The score itself, given the most room because it is the headline.
    score_label = results_canvas.create_text(325, 180, text="FINAL SCORE", font=SMALL_FONT, fill=TEXT_DIM)
    score_txt = results_canvas.create_text(325, 235, text=str(finished.score), font=SCORE_BIG_FONT, fill=ACCENT)
    ...
    for heading, value in stats:
        spot_x = 133 + column * 128
        results_canvas.create_text(spot_x, 312, text=heading, font=SMALL_FONT, fill=TEXT_DIM)
        results_canvas.create_text(spot_x, 340, text=value, font=STAT_FONT, fill=TEXT)
        column = column + 1

    # A bar showing the accuracy, so the number has something to sit against.
    bar_fill = 120 + (530 - 120) * finished.accuracy() / 100
    accuracy_track = results_canvas.create_rectangle(120, 388, 530, 400, fill=PANEL_EDGE, outline="")
    accuracy_bar = results_canvas.create_rectangle(120, 388, bar_fill, 400, fill=ACCENT, outline="")

    # A line of encouragement chosen from how the race actually went.
    message_txt = results_canvas.create_text(325, 435, text=result_message(finished.accuracy()), font=LABEL_FONT, fill=TEXT, width=460)

    # Function for the race again button, which starts a fresh race.
    def race_again():
        results_win.destroy()
        race()

    # Function for the menu button, which brings the main menu back.
    def to_menu():
        results_win.destroy()
        root.deiconify()

    # Set up the two buttons under the card.
    again_btn = Button(results_win, text="Race Again", font=BUTTON_FONT, width=16, height=2, command=race_again, bg=ACCENT, fg=BACKGROUND, activebackground=ACCENT_DEEP, activeforeground=BACKGROUND, relief=FLAT, bd=0, cursor="hand2")
    again_win = results_canvas.create_window(215, 540, anchor=CENTER, window=again_btn)
    menu_btn = Button(results_win, text=" Main Menu", font=BUTTON_FONT, image=back_img, compound=LEFT, command=to_menu, bg=BUTTON_BG, fg=TEXT, activebackground=BUTTON_ACTIVE, activeforeground=ACCENT, relief=FLAT, bd=0, cursor="hand2", padx=10, pady=10)
    menu_win = results_canvas.create_window(440, 540, anchor=CENTER, window=menu_btn)

    # Send the window X button back to the menu so it cannot get stranded.
    results_win.protocol("WM_DELETE_WINDOW", to_menu)
...
def race():
    ...
    # Paint the solid lines down either kerb of the road.
    race_canvas.create_line(ROAD_EDGE, ROAD_TOP, ROAD_EDGE, ROAD_BOTTOM, fill=EDGE_COLOUR, width=6)
    race_canvas.create_line(650 - ROAD_EDGE, ROAD_TOP, 650 - ROAD_EDGE, ROAD_BOTTOM, fill=EDGE_COLOUR, width=6)

    # Paint the dashed lines dividing the three lanes. They are kept in a list
    # so their dashes can be scrolled to make the road look like it is moving.
    lane_lines = []
    for divider in range(1, LANE_COUNT):
        line_x = (LANE_X[divider - 1] + LANE_X[divider]) / 2
        lane_lines.append(race_canvas.create_line(line_x, ROAD_TOP, line_x, ROAD_BOTTOM, fill=LANE_LINE_COLOUR, width=4, dash=LANE_DASH))

    # Set up the bar across the top holding the question and the score.
    header_panel = race_canvas.create_rectangle(0, 0, 650, HEADER_BOTTOM, fill=BACKGROUND, outline="")
    question_txt = race_canvas.create_text(325, QUESTION_Y, text="", font=QUESTION_FONT, fill=TEXT, width=600, justify=CENTER)
    scoreboard_txt = race_canvas.create_text(325, SCOREBOARD_Y, text="", font=SCORE_FONT, fill=ACCENT)
    instruction_txt = race_canvas.create_text(325, INSTRUCTION_Y, text=DRIVING_HELP, font=SMALL_FONT, fill=TEXT_DIM)

    # Set up the bar that fills up as the race is worked through.
    progress_track = race_canvas.create_rectangle(PROGRESS_LEFT, PROGRESS_TOP, PROGRESS_RIGHT, PROGRESS_BOTTOM, fill=PANEL_EDGE, outline="")
    progress_bar = race_canvas.create_rectangle(PROGRESS_LEFT, PROGRESS_TOP, PROGRESS_LEFT, PROGRESS_BOTTOM, fill=ACCENT, outline="")

    # Set up the matching bar along the bottom for the back button and result.
    footer_panel = race_canvas.create_rectangle(0, ROAD_BOTTOM, 650, 650, fill=BACKGROUND, outline="")
    feedback_txt = race_canvas.create_text(400, BOTTOM_ROW_Y, text="", font=SCORE_FONT, fill=TEXT)
    ...
    # Set up the bike, which stays on its own row and only changes lane.
    ...
    # True while the gates are on their way down, so a run is under way.
    def gates_are_moving():
        return race_canvas.coords(gate_texts[0])[1] > GATE_Y

    # Line all three gates up across the road at the given height.
    def place_gates(row):
        for lane in range(LANE_COUNT):
            race_canvas.coords(gate_texts[lane], LANE_X[lane], row)
    ...
    def highlight_lane():
        for lane in range(LANE_COUNT):
            if lane == bike_lane():
                race_canvas.itemconfig(gates[lane], outline=ACCENT, width=5)
            else:
                race_canvas.itemconfig(gates[lane], outline=LANE_LINE_COLOUR, width=3)
    ...
    def show_scoreboard():
        ...
        filled = PROGRESS_LEFT + (PROGRESS_RIGHT - PROGRESS_LEFT) * game.asked / game.total
        race_canvas.coords(progress_bar, PROGRESS_LEFT, PROGRESS_TOP, filled, PROGRESS_BOTTOM)
    ...
    def show_question():
        ...
        for lane in range(LANE_COUNT):
            race_canvas.itemconfig(gate_texts[lane], text=choices[lane], fill=TEXT)
            race_canvas.itemconfig(gates[lane], fill=GATE_COLOUR)
    ...
    def scroll_road():
        offset = int(float(race_canvas.itemcget(lane_lines[0], "dashoffset")))
        for line in lane_lines:
            race_canvas.itemconfig(line, dashoffset=(offset + drive_step) % DASH_CYCLE)

    # Close the road down and hand the finished race to the results window.
    def finish_race():

        # Give up if the window was closed while the last message was showing.
        if not race_win.winfo_exists():
            return

        # Save the run before the window closes, so it reaches the leaderboard.
        save_record(game.difficulty, signed_in_user, game.score, game.seconds())
    ...
    def start_next():
        ...
        # Give up if the window was closed while the message was showing.
        if not race_win.winfo_exists():
            return

        game.next_question()
        show_question()
        race_canvas.itemconfig(feedback_txt, text="")

    # Mark whichever gate reached the bike, then line up the next question.
    def arrive():
        chosen_lane = bike_lane()
        chosen = race_canvas.itemcget(gate_texts[chosen_lane], "text")
        ...
        if game.mark(chosen):
            ...
            race_canvas.itemconfig(gates[chosen_lane], fill=WRONG_COLOUR)
            race_canvas.itemconfig(gate_texts[chosen_lane], fill=BACKGROUND)

        # Colour the right gate green either way, so a wrong answer still
        # shows the player where the correct one was.
        for lane in range(LANE_COUNT):
            if race_canvas.itemcget(gate_texts[lane], "text") == right_answer:
                race_canvas.itemconfig(gates[lane], fill=CORRECT_COLOUR)
                race_canvas.itemconfig(gate_texts[lane], fill=BACKGROUND)

        show_scoreboard()

        # Either way the message stays up for a moment before moving on, so
        # the last answer of a race is not swallowed by the results window.
        if game.is_finished():
            race_win.after(FEEDBACK_PAUSE, finish_race)
        else:
            race_win.after(FEEDBACK_PAUSE, start_next)

    # Move the gates one frame down the road towards the waiting bike.
    def drive_down():

        # Give up if the window was closed part way through a run.
        if not race_win.winfo_exists():
            return

        row = race_canvas.coords(gate_texts[0])[1] + drive_step
        scroll_road()

        # Keep coming until the gates have pulled up in front of the bike.
        if row < GATE_STOP_Y:
            place_gates(row)
            race_win.after(FRAME_DELAY, drive_down)
            return

        place_gates(GATE_STOP_Y)
        arrive()

    # Function for the space bar and the enter key, which sets the gates off.
    def submit(event):

        # Ignore the key while a run is already under way or the race is over.
        if gates_are_moving() or game.is_finished():
            return
    ...
    back_win = race_canvas.create_window(85, BOTTOM_ROW_Y, anchor=CENTER, window=back_btn)

    # Steer with the arrow keys and set off with space or enter. There is no
    # answer box on this screen, so the arrows are free to be used for driving.
    race_win.bind("<Left>", go_left)
    race_win.bind("<Right>", go_right)
    race_win.bind("<space>", submit)
    race_win.bind("<Return>", submit)
    race_win.focus_set()
    ...
    # Put the first question up and wait for the player to set off.
    show_question()
...
def leaderboard():
    ...
    # Hide the main menu so only the leaderboard is on screen.
    root.withdraw()

    board_win = tk.Toplevel(root)
    board_win.title("Leaderboard")
    board_win.geometry("700x700")

    # Set up the canvas widget for the leaderboard window.
    board_canvas = Canvas(board_win, width=650, height=650, bg=BACKGROUND, highlightthickness=0)
    board_canvas.pack(pady=5)
    board_shadow = board_canvas.create_text(328, 64, text="LEADERBOARD", font=HEADING_FONT, fill=SHADOW)
    board_txt = board_canvas.create_text(325, 60, text="LEADERBOARD", font=HEADING_FONT, fill=ACCENT)

    # Draw one board for each difficulty, side by side across the window.
    column = 0
    for level in TOPICS:
        ...
        board_canvas.create_rectangle(board_left, BOARD_TOP, board_right, BOARD_BOTTOM, fill=PANEL, outline=PANEL_EDGE, width=2)
        board_canvas.create_text((board_left + board_right) / 2, BOARD_TOP + 34, text=level.upper(), font=BAND_FONT, fill=ZONE_COLOURS[level])

        # The column headings, which sit above a dividing line.
        board_canvas.create_text(board_left + NAME_OFFSET, BOARD_TOP + 68, text="DRIVER", font=SMALL_FONT, fill=TEXT_DIM, anchor=W)
        board_canvas.create_text(board_left + SCORE_OFFSET, BOARD_TOP + 68, text="PTS", font=SMALL_FONT, fill=TEXT_DIM, anchor=E)
        board_canvas.create_text(board_left + TIME_OFFSET, BOARD_TOP + 68, text="TIME", font=SMALL_FONT, fill=TEXT_DIM, anchor=E)
        board_canvas.create_line(board_left + 10, BOARD_TOP + 82, board_right - 10, BOARD_TOP + 82, fill=PANEL_EDGE, width=2)

        rows = best_records(level, LEADERBOARD_ROWS)

        # An empty board says so rather than leaving a blank card.
        if len(rows) == 0:
            board_canvas.create_text((board_left + board_right) / 2, ROW_TOP + 60, text="No races yet", font=SMALL_FONT, fill=TEXT_DIM)
        ...
        for level_name, driver, score, seconds in rows:
            ...
            if driver == signed_in_user:
                row_colour = ACCENT
            else:
                row_colour = TEXT

            board_canvas.create_text(board_left + NAME_OFFSET, row_y, text=str(place + 1) + "  " + driver, font=SMALL_FONT, fill=row_colour, anchor=W)
            board_canvas.create_text(board_left + SCORE_OFFSET, row_y, text=str(score), font=SMALL_FONT, fill=row_colour, anchor=E)
            board_canvas.create_text(board_left + TIME_OFFSET, row_y, text=format_time(seconds), font=SMALL_FONT, fill=row_colour, anchor=E)
            place = place + 1

        column = column + 1

    # Explain the ordering, since equal scores are split by the clock.
    board_canvas.create_text(325, 578, text="Best score first. A tie is settled by the quicker time. Only each driver's best run is shown.", font=SMALL_FONT, fill=TEXT_DIM)
    ...
    # Set up the back button below the boards.
    back_btn = Button(board_win, text=" Back", font=BUTTON_FONT, image=back_img, compound=LEFT, command=back, bg=BUTTON_BG, fg=TEXT, activebackground=BUTTON_ACTIVE, activeforeground=ACCENT, relief=FLAT, bd=0, cursor="hand2", padx=8, pady=6)
    back_win = board_canvas.create_window(110, 618, anchor=CENTER, window=back_btn)

    # Send the window X button through the back function so the menu cannot get stranded.
    board_win.protocol("WM_DELETE_WINDOW", back)
